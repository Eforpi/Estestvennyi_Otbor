...
public class AI : MonoBehaviour
{
    ...
    [Range(0f,2f)]
    public float mutationsScale;
    public MainController controller;

    [Range(0f,10f)]
    public float defaultVision;
    [Range(0f,1)]
    public float starveSpeed;
    void Start()
    {
    ...
    void FixedUpdate()
    {
        float vision = defaultVision + attackSkill;
        ...
        rb.MoveRotation(Mathf.Atan2(rb.velocity.y, rb.velocity.x) * Mathf.Rad2Deg - 90);
        rb.velocity = velocity;

        energy -= Time.deltaTime*(foodSkill+attackSkill+defSkill)*starveSpeed;
        if(energy < 0f)
        {
            Kill();
        }
    ...
    private void Eat(float food)
    {
        ...
        {
            energy *= 0.5f;
            GameObject b = (GameObject)Object.Instantiate(Resources.Load("entity", typeof(GameObject)), new Vector3(0, 0, 0), Quaternion.identity);
            controller.bacCount +=1;
            b.transform.position = transform.position;
            b.name = "bacterium";
            Genome g = new Genome(genome);
            g.Mutate(0.5f,mutationsScale);
