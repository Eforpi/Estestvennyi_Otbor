...
public class Genome
{
    ...
    public void Mutate(float value,float mutationsScale)
    {
        for(int i = 0; i < weights.Length; i++)
        {
            if(UnityEngine.Random.value < 0.1*mutationsScale) weights[i] += UnityEngine.Random.Range(-value, value);
        }
        for(int i = 0; i < skillCount; i++)
        {
            if(UnityEngine.Random.value < 0.05*mutationsScale) {
